...
START_TEST(sub_normal_test) {
  ...
  for (int i = 0; i < A.rows; i++) {
    for (int j = 0; j < A.columns; j++) {
      ...
      expected = accuracy_check(A.matrix[i][j], ACCURACY) -
                 accuracy_check(B.matrix[i][j], ACCURACY);
...
static int rows_edge[] = {3, 5, 5};
static int cols_edge[] = {3, 4, 5};

static double A_filler_edge[] = {0, 0, 0};
static double B_filler_edge[] = {0, 0.00000001, -0};
...
Suite *suite_s21_sub_matrix() {
  ...
  tcase_add_loop_test(normal_case, sub_normal_test, 0,
                      sizeof(rows_normal) / sizeof(int));
  suite_add_tcase(suite, normal_case);

  TCase *edge_case = tcase_create("case_s21_sub_matrix_edge");
  tcase_add_loop_test(edge_case, sub_edge_test, 0,
                      sizeof(rows_edge) / sizeof(int));
