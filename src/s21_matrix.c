...
int s21_create_matrix(int rows, int columns, matrix_t *result) {
  ...
  for (int i = 0; (i < rows) && (res == OK); i++) {
    ...
    if (result->matrix[i] == NULL) res = INVALID_MATRIX;
  }
...
int s21_sum_matrix(matrix_t *A, matrix_t *B, matrix_t *result) {
  ...
  if (res == OK) {
    for (int i = 0; i < A->rows; i++) {
      for (int j = 0; j < A->columns; j++) {
        result->matrix[i][j] = accuracy_check(A->matrix[i][j], ACCURACY) +
...
int s21_mult_number(matrix_t *A, double number, matrix_t *result) {
  ...
  if (res == OK) {
    ...
    for (int i = 0; i < A->rows; i++) {
      for (int j = 0; j < A->columns; j++) {
        result->matrix[i][j] =
            accuracy_check(A->matrix[i][j], ACCURACY) * number;
      }
    }
  }
    ...
    for (int i = 0; i < A->rows; i++) {
      for (int j = 0; j < B->columns; j++) {
        ...
        for (int k = 0; k < A->columns; k++) {
          sum += accuracy_check(A->matrix[i][k], ACCURACY) *
...
int s21_determinant(matrix_t *A, double *result) {
  ...
  if (A->rows == 1) {
    ...
    *result = accuracy_check(A->matrix[0][0], ACCURACY) *
                  accuracy_check(A->matrix[1][1], ACCURACY) -
              accuracy_check(A->matrix[0][1], ACCURACY) *
                  accuracy_check(A->matrix[1][0], ACCURACY);
  } else {
    matrix_t sub = {0};
